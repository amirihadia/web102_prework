...
function addGamesToPage(games) {
    ...
    for(let i=0; i<games.length; i++) {
        ...
        gameCard.innerHTML = `
            <img class="game-img" src="${game.img}" alt="${games.name}" />
            <h3> ${game.name}</h3>
...
const totalAmountRaised = GAMES_JSON.reduce((acc, games) =>{
...
const totalGames = GAMES_JSON.length; //since GAMES_JSON is just an array of all the game objects, the total
//number of games is just the length of the array
...
function filterUnfundedOnly() {
    ...
    //testing SECRET KEY COMPONENT 1
...
function filterFundedOnly() {
    ...
    //testing SECRET KEY COMPONENT 2
    return funded.length;
}

// show all games
function showAllGames() {
    deleteChildElements(gamesContainer); //clears whatever is the in container
...
const numUnfunded = GAMES_JSON.filter(games => games.pledged < games.goal).length;

// create a string that explains the number of unfunded games using the ternary operator
const numFunded = GAMES_JSON.length - numUnfunded;
let statement = `A total of $${totalAmountRaised.toLocaleString()} has been raised for ${numFunded} ${numFunded === 1 ? "game" : "games"}.
...
descriptionParagraph.textContent = statement;
...
//this sorted the games by pledge
...
//temporarily added to see the output of the console.log statements 
